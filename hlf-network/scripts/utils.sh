...
function printHelp() {
  ...
  if [ "$USAGE" == "up" ]; then
    ...
    println "    -ccep <policy>  - (Optional) Chaincode endorsement policy using signature policy syntax. The default policy requires an endorsement from manufacturer and retailer"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c mychannel -s couchdb -i 2.2.3"
    println "   network.sh createChannel -c channelName"
    println "   network.sh deployCC -ccn tfbc -ccp ../chaincode/tfbc/go -ccl go"
  fi
}
